"""
fix the opacity animations to also go from 0%-100%-0% like this example:
Note that it isn't instant visibility but a gradual, linear animation.

if I say animation is opacity-south I expect an animation from top to bottom, just as the scale-south does. please fix.

IT'S supposed to start at opposite side and go in the direction just as WEST goes from EAST to WEST.

the directions are all inverted

north can also be up or u
south can also be down or d
west can also be left or l
east can also be right or r
northeast can also be upright or ur
northwest can also be upleft or ul
southeast can also be downright or dr
...
def get_move_keyframes(animate_enabled, animation_type=None, width=0, height=0, duration=1.0):
    ...
    if animation_type == "move-north":
        # Start from below the canvas, end above the canvas (inverted)
        start_x, start_y = 0, height * 2
        end_x, end_y = 0, -height * 2
    elif animation_type == "move-south":
        # Start from above the canvas, end below the canvas (inverted)
        start_x, start_y = 0, -height * 2
        end_x, end_y = 0, height * 2
    elif animation_type == "move-east":
        # Start from the left of the canvas, end to the right (inverted)
        start_x, start_y = -width * 2, 0
        end_x, end_y = width * 2, 0
    elif animation_type == "move-west":
        # Start from the right of the canvas, end to the left (inverted)
        start_x, start_y = width * 2, 0
        end_x, end_y = -width * 2, 0
    elif animation_type == "move-northeast":
        # Start from southwest, end to northeast (inverted)
        start_x, start_y = -width * 2, height * 2
        end_x, end_y = width * 2, -height * 2
    elif animation_type == "move-northwest":
        # Start from southeast, end to northwest (inverted)
        start_x, start_y = width * 2, height * 2
        end_x, end_y = -width * 2, -height * 2
    elif animation_type == "move-southeast":
        # Start from northwest, end to southeast (inverted)
        start_x, start_y = -width * 2, -height * 2
        end_x, end_y = width * 2, height * 2
    elif animation_type == "move-southwest":
        # Start from northeast, end to southwest (inverted)
        start_x, start_y = width * 2, -height * 2
        end_x, end_y = -width * 2, height * 2
    else:
        # Default: no movement
...
def get_opacity_keyframes(animate_enabled, animation_type=None, row=0, col=0, rows=1, cols=1, duration=1.0):
    ...
    if not animate_enabled or not animation_type or (not animation_type.startswith("opacity-") and animation_type != "opacity"):
        return {"value": 1}
    ...
    if animation_type == "opacity":
        # Uniform opacity animation for all tiles
        return {
            "keyframes": [
                {
                    "after": {"x": 1, "y": 1},
        ...
        # Calculate keyframe times based on position
        # Animation timeline:
        # 0.0: start at 0
        # position * 0.5: start fading in
        # 0.5: reach 100%
        # 0.5 + position * 0.5: start fading out
        # 1.0: back to 0
