"""
Well done, now do it for RAWR.

this did not work correctly (python tiler23.py -F rawr -c 4 -a scale-west) fix it

python tiler23.py -F rawr -c 4 -a scale-west should give me the same animation on either SVG or RAWR

ok but why does a regular scale animation have so many tiles, it only needs one
"""
import argparse
import math
import json
...
def get_scale_keyframes(animate_enabled, animation_type=None, row=0, col=0, rows=1, cols=1):
    ...
    if animation_type == "scale" or animation_type is None:
        ...
        inflation_start = position * 30  # 30 frames = 0.5 seconds at 60fps
        deflation_start = 30 + position * 30  # 30 frames = 0.5 seconds at 60fps
...
def generate_triangular_tiling(width, height, size, rotation, fill, stroke, stroke_width, animate=None):
    ...
    # For directional scaling, we create compositions without animation
    ...
    if animate_enabled and (animation_type == "scale" or animation_type is None):
        ...
        upright_triangle_composition["shapes"][0]["transform"]["scale"] = {
            "keyframes": [
                {
                    "after": {"x": 1, "y": 1},
                    "before": {"x": 0, "y": 0},
        ...
        inverted_triangle_composition["shapes"][0]["transform"]["scale"] = {
            "keyframes": [
                {
                    "after": {"x": 1, "y": 1},
                    "before": {"x": 0, "y": 0},
    ...
    # For directional scaling, create row compositions
    row_compositions = {}
    if animate_enabled and animation_type != "scale" and animation_type is not None:
        # Create a composition for each row
        for row in range(rows):
            ...
            row_compositions[row] = row_composition
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if is_triangle_inside_after_rotation(x_main, y_main, size, inverted, width, height, rotation):
                ...
                if animate_enabled and animation_type != "scale" and animation_type is not None:
                    # For directional scaling, add to row composition
                    row_comp = row_compositions[row]
                    
                    # Create a layer for this triangle in the row composition
                    triangle_layer = {
                        "__type__": "Layer",
                        "animation": {
                            "__type__": "AnimationContainer",
                    ...
                    row_comp["shapes"].append(triangle_layer)
                else:
                    # For uniform scaling or no animation, add directly to grid
                    precomp_layer = {
                        "__type__": "PreCompLayer",
                        "composition": triangle_uuid,
                        "group_color": "#00000000",
    ...
    # Add row compositions to the grid for directional scaling
    if animate_enabled and animation_type != "scale" and animation_type is not None:
        for row, row_comp in row_compositions.items():
            if row_comp["shapes"]:  # Only add if the row has tiles
                # Add animation to the row composition
                row_comp["shapes"][0]["transform"]["scale"] = get_scale_keyframes(
                    animate_enabled, animation_type, row, 0, rows, 1
                )
                
                # Create a precomposed layer for this row
                row_layer = {
                    "__type__": "PreCompLayer",
                    "composition": row_comp["uuid"],
                    "group_color": "#00000000",
                    "locked": False,
                    "name": f"Row {row}",
                    "opacity": {"value": 1},
                    "size": {"height": triangle_height, "width": expanded_width},
                    "timing": {
                        "__type__": "StretchableTime",
                        "start_time": 0,
                        "stretch": 1
                ...
                grid_composition["shapes"].append(row_layer)
    ...
    # Add row compositions for directional scaling
    if animate_enabled and animation_type != "scale" and animation_type is not None:
        for row_comp in row_compositions.values():
            if row_comp["shapes"]:  # Only add if the row has tiles
                glaxnimate_data["assets"]["compositions"]["values"].append(row_comp)
...
def generate_rawr_square_tiling(width, height, corners, tile_size, rotation, fill, stroke, stroke_width, animate=None):
    ...
    if animate_enabled and (animation_type == "scale" or animation_type is None):
        ...
        tile_composition["shapes"][0]["transform"]["scale"] = {
            "keyframes": [
                {
                    "after": {"x": 1, "y": 1},
                    "before": {"x": 0, "y": 0},
    ...
    # For directional scaling, create row or column compositions
    line_compositions = {}
    if animate_enabled and animation_type != "scale" and animation_type is not None:
        # Determine if we should group by rows or columns
        if animation_type in ["scale-north", "scale-south"]:
            # Group by rows
            for row in range(rows):
                ...
                line_compositions[row] = line_composition
        elif animation_type in ["scale-east", "scale-west"]:
            # Group by columns
            for col in range(cols):
                line_uuid = str(uuid.uuid4())
                line_composition = {
                ...
                line_compositions[col] = line_composition
        else:
            # For diagonal directions, create one composition per tile
            pass
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if is_square_inside_after_rotation(x_main, y_main, tile_size, width, height, rotation):
                if animate_enabled and animation_type != "scale" and animation_type is not None:
                    if animation_type in ["scale-north", "scale-south", "scale-east", "scale-west"]:
                        # For row or column based directions, add to line composition
                        if animation_type in ["scale-north", "scale-south"]:
                            line_comp = line_compositions[row]
                        else:
                            line_comp = line_compositions[col]
                        
                        # Create a layer for this tile in the line composition
                        tile_layer = {
                            "__type__": "Layer",
                            "animation": {
                                "__type__": "AnimationContainer",
                        ...
                        line_comp["shapes"].append(tile_layer)
                    else:
                        # For diagonal directions, add directly to grid with individual animation
                        precomp_layer = {
                            "__type__": "PreCompLayer",
                            "composition": tile_uuid,
                            "group_color": "#00000000",
                            ...
                            "transform": {
                                ...
                                "position": {"value": {"x": x, "y": y}},
                                "rotation": {"value": 0},
                                "scale": get_scale_keyframes(animate_enabled, animation_type, row, col, rows, cols)
                            },
                            "uuid": str(uuid.uuid4()),
                            "visible": True
                        }
    ...
    # Add line compositions to the grid for directional scaling
    if animate_enabled and animation_type != "scale" and animation_type is not None:
        if animation_type in ["scale-north", "scale-south", "scale-east", "scale-west"]:
            for line_idx, line_comp in line_compositions.items():
                if line_comp["shapes"]:  # Only add if the line has tiles
                    # Add animation to the line composition
                    if animation_type in ["scale-north", "scale-south"]:
                        line_comp["shapes"][0]["transform"]["scale"] = get_scale_keyframes(
                            animate_enabled, animation_type, line_idx, 0, rows, 1
                        )
                    else:  # scale-east or scale-west
                        line_comp["shapes"][0]["transform"]["scale"] = get_scale_keyframes(
                            animate_enabled, animation_type, 0, line_idx, 1, cols
                        )
                    
                    # Create a precomposed layer for this line
                    line_layer = {
                        "__type__": "PreCompLayer",
                        "composition": line_comp["uuid"],
                        "group_color": "#00000000",
                        "locked": False,
                        "name": line_comp["name"],
                        "opacity": {"value": 1},
                        "size": {"height": line_comp["height"], "width": line_comp["width"]},
                        "timing": {
                            "__type__": "StretchableTime",
                            "start_time": 0,
                            "stretch": 1
                    ...
                    grid_composition["shapes"].append(line_layer)
    ...
    # Add line compositions for directional scaling
    if animate_enabled and animation_type != "scale" and animation_type is not None:
        if animation_type in ["scale-north", "scale-south", "scale-east", "scale-west"]:
            for line_comp in line_compositions.values():
                if line_comp["shapes"]:  # Only add if the line has tiles
                    glaxnimate_data["assets"]["compositions"]["values"].append(line_comp)
...
def generate_rawr_hexagonal_tiling(width, height, corners, tile_size, rotation, fill, stroke, stroke_width, animate=None):
    ...
    if animate_enabled and (animation_type == "scale" or animation_type is None):
        ...
        hex_composition["shapes"][0]["transform"]["scale"] = {
            "keyframes": [
                {
                    "after": {"x": 1, "y": 1},
                    "before": {"x": 0, "y": 0},
        ...
        for col in range(cols):
            ...
            if is_hexagon_inside_after_rotation(x_main, y_main, hex_radius, width, height, rotation):
                if animate_enabled and animation_type != "scale" and animation_type is not None:
                    ...
                    # Create a layer for this hexagon in the row composition
                    hex_layer = {
                        "__type__": "Layer",
                        "animation": {
                            "__type__": "AnimationContainer",
                    ...
                    row_comp["shapes"].append(hex_layer)
                else:
                    # For uniform scaling or no animation, add directly to grid
                    precomp_layer = {
                        "__type__": "PreCompLayer",
        ...
        for row, row_comp in row_compositions.items():
            if row_comp["shapes"]:  # Only add if the row has tiles
                ...
                row_layer = {
                    ...
                    "name": f"Row {row}",
                    "opacity": {"value": 1},
                    "size": {"height": hex_radius * 2, "width": expanded_width},
                    "timing": {
                        "__type__": "StretchableTime",
                        "start_time": 0,
                        "stretch": 1
