"""
fix the animations to not rotate, in other words, "python tiler.py -c 4 -r 45 -F rawr -a scale-south" should still animate 45º tiles from top to bottom.
"""
import argparse
import math
import json
...
def get_animation_keyframes(row, col, rows, cols, direction, duration, rotation=0):
...
def get_move_animation_values(width, height, direction):
    """Get the animation values for directional movement of the entire grid."""
    if direction == "move-north":
        # Start from below the canvas, move to center, then continue above
        return f"0,{height * 2}; 0,0; 0,-{height * 2}"
    elif direction == "move-south":
        # Start from above the canvas, move to center, then continue below
        return f"0,-{height * 2}; 0,0; 0,{height * 2}"
    elif direction == "move-east":
        # Start from the left of the canvas, move to center, then continue right
        return f"-{width * 2},0; 0,0; {width * 2},0"
    elif direction == "move-west":
        # Start from the right of the canvas, move to center, then continue left
        return f"{width * 2},0; 0,0; -{width * 2},0"
    elif direction == "move-northeast":
        # Start from southwest, move to center, then continue northeast
        return f"-{width * 2},{height * 2}; 0,0; {width * 2},-{height * 2}"
    elif direction == "move-northwest":
        # Start from southeast, move to center, then continue northwest
        return f"{width * 2},{height * 2}; 0,0; -{width * 2},-{height * 2}"
    elif direction == "move-southeast":
        # Start from northwest, move to center, then continue southeast
        return f"-{width * 2},-{height * 2}; 0,0; {width * 2},{height * 2}"
    elif direction == "move-southwest":
        # Start from northeast, move to center, then continue southwest
        return f"{width * 2},-{height * 2}; 0,0; -{width * 2},{height * 2}"
    else:
        # Default: no movement
        return "0,0; 0,0; 0,0"

def get_opacity_animation_values(row, col, rows, cols, direction, duration):
    """Get opacity animation values for clear-fill-clear animations."""
    # Calculate normalized position (0 to 1) based on direction
    if direction == "opacity-north":
        # Start at bottom (south) and move to top (north)
        position = (rows - 1 - row) / (rows - 1) if rows > 1 else 0
    elif direction == "opacity-south":
        # Start at top (north) and move to bottom (south)
        position = row / (rows - 1) if rows > 1 else 0
    elif direction == "opacity-east":
        # Start at left (west) and move to right (east)
        position = col / (cols - 1) if cols > 1 else 0
    elif direction == "opacity-west":
        # Start at right (east) and move to left (west)
        position = (cols - 1 - col) / (cols - 1) if cols > 1 else 0
    elif direction == "opacity-northeast":
        # Start at southwest and move to northeast
        position = ((rows - 1 - row) + col) / (rows - 1 + cols - 1) if (rows > 1 or cols > 1) else 0
    elif direction == "opacity-northwest":
        # Start at southeast and move to northwest
        position = ((rows - 1 - row) + (cols - 1 - col)) / (rows - 1 + cols - 1) if (rows > 1 or cols > 1) else 0
    elif direction == "opacity-southeast":
        # Start at northwest and move to southeast
        position = (row + col) / (rows - 1 + cols - 1) if (rows > 1 or cols > 1) else 0
    elif direction == "opacity-southwest":
        # Start at northeast and move to southwest
        position = (row + (cols - 1 - col)) / (rows - 1 + cols - 1) if (rows > 1 or cols > 1) else 0
    elif direction == "opacity":
        # SIMPLE SPIRAL: Calculate distance and angle from center
        center_row = (rows - 1) / 2
        center_col = (cols - 1) / 2
        ...
        distance_factor = distance / max_distance if max_distance > 0 else 0
        
        # Calculate angle from center (0 to 1)
        angle = math.atan2(dy, dx)
        if angle < 0:
            angle += 2 * math.pi
        angle_factor = angle / (2 * math.pi)
        
        # Combine for spiral effect (angle matters more for spiral pattern)
        spiral_position = (angle_factor * 0.7 + distance_factor * 0.3) % 1.0
        
        # Fade in: 0 to spiral_position
        # Stay opaque: spiral_position to spiral_position + 0.3
        # Fade out: spiral_position + 0.3 to 1
        fade_in_start = 0
        ...
        key_times = f"{fade_in_start}; {fade_in_end}; {opaque_start}; {opaque_end}; {fade_out_start}; {fade_out_end}"
        values = "0; 1; 1; 1; 1; 0"
...
def get_move_keyframes(animate_enabled, animation_type=None, width=0, height=0, duration=1.0):
    ...
    if animation_type == "move-north":
        # Start from below the canvas, end above the canvas
        start_x, start_y = 0, height * 2
        end_x, end_y = 0, -height * 2
    elif animation_type == "move-south":
        # Start from above the canvas, end below the canvas
        start_x, start_y = 0, -height * 2
        end_x, end_y = 0, height * 2
    elif animation_type == "move-east":
        # Start from the left of the canvas, end to the right
        start_x, start_y = -width * 2, 0
        end_x, end_y = width * 2, 0
    elif animation_type == "move-west":
        # Start from the right of the canvas, end to the left
        start_x, start_y = width * 2, 0
        end_x, end_y = -width * 2, 0
    elif animation_type == "move-northeast":
        # Start from southwest, end to northeast
        start_x, start_y = -width * 2, height * 2
        end_x, end_y = width * 2, -height * 2
    elif animation_type == "move-northwest":
        # Start from southeast, end to northwest
        start_x, start_y = width * 2, height * 2
        end_x, end_y = -width * 2, -height * 2
    elif animation_type == "move-southeast":
        # Start from northwest, end to southeast
        start_x, start_y = -width * 2, -height * 2
        end_x, end_y = width * 2, height * 2
    elif animation_type == "move-southwest":
        # Start from northeast, end to southwest
        start_x, start_y = width * 2, -height * 2
        end_x, end_y = -width * 2, height * 2
    else:
        # Default: no movement
...
def get_opacity_keyframes(animate_enabled, animation_type=None, row=0, col=0, rows=1, cols=1, duration=1.0):
    """Get the opacity keyframes for RAWR format clear-fill-clear animations."""
    if not animate_enabled or not animation_type or not animation_type.startswith("opacity-"):
        return {"value": 1}
    ...
    if animation_type == "opacity-north":
        ...
        # Convert to frames
        fade_in_start = 0
        fade_in_end = int(spiral_position * total_frames * 0.35)
        opaque_start = fade_in_end
        opaque_end = fade_in_end + int(total_frames * 0.3)
        fade_out_start = opaque_end
    ...
    appear_start = int(position * total_frames * 0.6)
    fully_visible = appear_start + int(total_frames * 0.2)
    disappear_start = appear_start + int(total_frames * 0.4)
...
def generate_triangular_tiling_svg(width, height, size, rotation, fill, stroke, stroke_width, animate=None, duration=1.0):
    ...
    if animate_enabled and animation_type and animation_type.startswith("move-"):
        move_values = get_move_animation_values(width, height, animation_type)
        svg_content += f'<g>\n'
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            # This is the key fix - we need to calculate where this tile appears to the viewer
            ...
            if animate_enabled and animation_type and animation_type.startswith("scale"):
                ...
                if animation_type == "scale":
                    ...
                    # Directional scaling - use viewer's coordinate system
                    key_times, values = get_animation_keyframes(viewer_row, viewer_col, rows, cols, animation_type, duration, rotation)
                    svg_content += f'<animateTransform attributeName="transform" type="scale" '
                ...
                svg_content += f'</g>\n'
            elif animate_enabled and animation_type and animation_type.startswith("opacity"):
                # Add opacity animation to the triangle for clear-fill-clear effect
                svg_content += f'<g transform="translate({x},{y})">\n'
                
                # Get opacity animation values - use viewer's coordinate system
                key_times, values = get_opacity_animation_values(viewer_row, viewer_col, rows, cols, animation_type, duration)
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if is_triangle_inside_after_rotation(x_main, y_main, size, inverted, width, height, rotation):
                ...
                if animate_enabled and animation_type and animation_type.startswith("opacity-"):
                    # Calculate the final position in the canvas after rotation
                    final_x, final_y = rotate_point(x_main, y_main, width/2, height/2, rotation)
...
def main():
    ...
    parser.add_argument('-a', '--animate', nargs='?', const='scale', 
                        choices=["scale", "scale-north", "scale-south", "scale-east", "scale-west", 
                                 ...
                                 "opacity-north", "opacity-south", "opacity-east", "opacity-west",
                                 "opacity-northeast", "opacity-northwest", "opacity-southeast", "opacity-southwest",
                                 "opacity"],
