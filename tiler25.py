"""
The directions are inverted, for example scale southeast should start northwest, fill up to southeast then deflate all formats.
"""
import argparse
import math
import json
...
def generate_triangular_tiling_svg(width, height, size, rotation, fill, stroke, stroke_width, animate=None):
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if animate_enabled:
                # Add animation to the triangle with proper centering
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if is_square_inside_after_rotation(x, y, size, width, height, rotation):
                if animate_enabled:
                    # Add animation to the square with proper centering
                    svg_content += f'<g transform="translate({x},{y})">\n'
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if is_hexagon_inside_after_rotation(x, y, hex_radius, width, height, rotation):
                if animate_enabled:
                    # Add animation to the hexagon with proper centering
                    svg_content += f'<g transform="translate({x},{y})">\n'
