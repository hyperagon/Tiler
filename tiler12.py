"""
I don't want per-tile rotation, you need to rotate the whole thing and remove tiles outside of the document
"""
import argparse
import math
import json
...
def generate_rawr_hexagonal_tiling(width, height, corners, tile_size, rotation, fill, stroke, stroke_width, animate):
    ...
    # Calculate the size needed for the grid to cover the document after rotation
    # We need to account for the diagonal of the document when rotated
    diagonal = math.sqrt(width**2 + height**2)
    grid_width = diagonal
    grid_height = diagonal
    
    # Calculate how many tiles we need to cover the grid
    cols = math.ceil(grid_width / horizontal_spacing) + 2
    rows = math.ceil(grid_height / vertical_spacing) + 2
    
    # Starting position to center the pattern in the grid
    start_x = (grid_width - (cols-1) * horizontal_spacing) / 2
    start_y = (grid_height - (rows-1) * vertical_spacing) / 2
        ...
        "shapes": [
            {
                ...
                "opacity": {"value": 1},
                "size": {"height": grid_height, "width": grid_width},
                "timing": {
                    "__type__": "StretchableTime",
                    "start_time": 0,
                    "stretch": 1
                ...
                "transform": {
                    "__type__": "Transform",
                    "anchor_point": {"value": {"x": grid_width/2, "y": grid_height/2}},
