"""
Prompt:
make me a Pyrhon application (give me the source code) that creates an SVG file of a tile tiled along its limits given through arguments.
Arguments:
- Width and Height (1280x720) by default
- Number of corners of the tile: 3,4 or 6
 - Offset of the tiles.
"""
...
def generate_triangular_tiling(width, height, offset_x, offset_y, tile_width, tile_height, fill, stroke, stroke_width):
    ...
    # Generate tiles in a grid with alternating upright and inverted triangles
    for row in range(tiles_y):
        for col in range(tiles_x):
            x = start_x + col * spacing_x
            y = start_y + row * spacing_y
            
            # Determine if this triangle should be inverted
            # Create a checkerboard pattern of upright and inverted triangles
            inverted = (row + col) % 2 == 1
            
            svg_content += generate_triangle(x, y, tile_width, tile_height, inverted, fill, stroke, stroke_width) + '\n'
